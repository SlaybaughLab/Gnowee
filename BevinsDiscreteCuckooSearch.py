#######################################################################################################
#
# Module : BevinsDiscreteCuckooSearch.py
#
...
#            Modified to allow for fraction of top nest to be applied to Study and A operators for meta-
#            optimization purposes.  
#
# Author : James Bevins
#
# Last Modified: 29Nov15
#
###############################################################################################################################
...
def New_Nests(func,nests,timeline,S,stats):  
    ...
    for i in range(0,S.c,1):
        ...
        while nests[i+S.c].f>=nests[i].f:    
            ...
            for j in range(0,len(nests[i].d),1):
                dist[j]=round(m.sqrt((nests[i+S.c].d[j][0]-nests[i+S.c].d[nests[i+S.c].i][0])**2 + \
                                     (nests[i+S.c].d[j][1]-nests[i+S.c].d[nests[i+S.c].i][1])**2))
                if j==nests[i+S.c].i:
                    dist[j]=1E99
            ...
            if ind>nests[i+S.c].i+1 or ind<nests[i+S.c].i-1:
                if ind<nests[i+S.c].i:
                    nests[i+S.c].d[ind+1:nests[i+S.c].i+1]=reversed(nests[i+S.c].d[ind+1:nests[i+S.c].i+1])
                    (nests[i+S.c].f,g)=func(nests[i+S.c].d)
                else: 
                    nests[i+S.c].d[nests[i+S.c].i:ind]=reversed(nests[i+S.c].d[nests[i+S.c].i:ind])
                    (nests[i+S.c].f,g)=func(nests[i+S.c].d)
                feval+=1    #Update function eval counter
                stats.nne+=1
                nests[i+S.c].i=ind   # For the ith nest, update the index of the most recently visited city
            else:
                nests[i+S.c].i=ind   # For the ith nest, update the index of the most recently visited city
                break
        if nests[i+S.c].f<nests[i].f:
            stats.nnc+=1
    ...
    # Remove old nests
    del nests[S.c:len(nests)]     # Unclear if this is true to Zhou DCS algorithm
...
def Study_Operator(func,nests,timeline,S,stats):  
    ...
    feval=0
    for i in range(0,int(S.pt*len(nests)),1):
        # Initialize variables
        rand1=i
